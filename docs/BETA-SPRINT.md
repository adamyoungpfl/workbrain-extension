...
| `FlowProgress`'s home | **Judgment: it stays unlabelled** (Adam, 2026-08-28: *"get rid of the label. make the icon, the standard logo."*). It briefly wore "Home" under the mark. A product's own mark in the top-left returning you home is the most-learned control on the web and the one icon whose meaning does not depend on reading it — and the mark only works as that door if it is the mark people recognise, which a 24px silhouette was not. The exemption is VISUAL: `aria-label` is unchanged, so nothing is lost to a screen reader. Second judged exception to §1, beside FileTree's orb toggle. **V1.7 VB-39 is reversed with it** — the status mark is the graph again |
